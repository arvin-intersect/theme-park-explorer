...
const RosterCalendar = ({ onDayClick, departmentId = null }: RosterCalendarProps) => {
    ...
    if (isLoading && !isOutside) {
      return (
        <div className="relative flex h-full w-full flex-col items-start p-2 text-left rounded-lg bg-muted/50">
          ...
          <div className="w-full space-y-1 mt-2"><Skeleton className="w-full h-4"/><Skeleton className="w-full h-4"/></div>
        </div>
      );
    }
    if (!summary && !isOutside) {
      return (
        <div className="relative flex h-full w-full flex-col items-start p-2 text-left rounded-lg bg-muted/30 border border-dashed">
          <div className={`self-end font-bold text-sm mb-1 ${isToday ? "text-primary" : "text-foreground/70"}`}>{format(date, "d")}</div>
          <div className="flex-1 w-full space-y-1 text-xs text-muted-foreground"><div className="text-center">No data</div></div>
        </div>
      );
    }

    const rosterPct = summary && summary.target_staff_count > 0 ? summary.rostered_staff_count / summary.target_staff_count : 1;
    const urgency = getUrgency(rosterPct, date);
    const canClick = onDayClick && summary && urgency.level !== 'past';

    return (
      <div
        className={`relative flex h-full w-full flex-col items-start p-2 text-left transition-all duration-200 ${ canClick ? "cursor-pointer border-2 border-border/50 hover:border-primary hover:shadow-lg hover:scale-[1.02] rounded-lg bg-gradient-to-br from-card to-card/90" : "border-2 border-transparent rounded-lg" } ${isToday && !isOutside ? "ring-2 ring-primary/50 ring-offset-background" : ""}`}
        onClick={() => canClick && onDayClick(date, summary)}
      >
        <div className={`self-end font-bold text-sm mb-1 ${ isOutside ? "text-muted-foreground/30" : isToday ? "text-primary" : "text-foreground/70" }`}>{format(date, "d")}</div>
        {summary && !isOutside && summary.target_staff_count > 0 && (
          <div className="flex-1 w-full space-y-1 text-xs">
            <div className="flex justify-between items-center">
              <span>Roster:</span><span className="font-bold">{(rosterPct * 100).toFixed(0)}%</span>
            </div>
            <div className={`text-center p-1 rounded-md font-semibold ${urgency.color}`}>{urgency.label}</div>
          </div>
        )}
        {summary && !isOutside && summary.target_staff_count === 0 && (<div className="flex-1 w-full space-y-1 text-xs text-muted-foreground"><div className="text-center">Park Closed</div></div>)}
      </div>
    );
  };
  ...
  if (error) {
    return (
      <Card className="bg-destructive/10 border-destructive/20"><CardContent className="p-6 text-center">
          <div className="text-destructive font-semibold">Error loading calendar data</div>
          <div className="text-sm text-muted-foreground mt-2">{(error as Error)?.message || 'Failed to fetch roster data'}</div>
      </CardContent></Card>
    );
  }
  ...
  return (
    <Card className="bg-gradient-to-br from-card via-card/95 to-card/90 backdrop-blur-sm border-2 shadow-xl">
      <CardContent className="p-4 sm:p-6">
        <DayPicker month={currentMonth} onMonthChange={setCurrentMonth} showOutsideDays fixedWeeks
          classNames={{ root: "w-full", months: "w-full", month: "space-y-6 w-full", caption: "flex justify-center pt-1 relative items-center", caption_label: "text-lg font-medium", nav: "space-x-1 flex items-center", nav_button: "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100", table: "w-full border-collapse", head_row: "flex justify-around mb-2", head_cell: "text-muted-foreground font-bold text-sm rounded-md w-full capitalize", row: "flex w-full mt-2 justify-around gap-2", cell: "h-28 sm:h-32 w-full text-center text-sm p-0 relative focus-within:relative focus-within:z-20", day: "h-full w-full", day_today: "", day_outside: "opacity-30", }}
