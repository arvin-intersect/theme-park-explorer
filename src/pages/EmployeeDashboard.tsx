...
const EmployeeDashboard = () => {
  ...
  const { data: employee, isLoading: isLoadingDetails } = useQuery({
      ...
      <main className="container mx-auto px-4 py-8">
        <Card className="mb-8 p-6 bg-card/80 backdrop-blur-sm">
            ...
            <Select onValueChange={setSelectedEmployeeId} value={selectedEmployeeId || ''}>
                <SelectTrigger>
                    <SelectValue placeholder={isLoadingList ? "Loading employees..." : "Select an employee"} />
                </SelectTrigger>
                <SelectContent>
        ...
        {!selectedEmployeeId && (
            <div className="text-center text-muted-foreground mt-16">
                <p>Please select an employee from the dropdown above to see their details.</p>
            </div>
        )}

        {isLoadingDetails && selectedEmployeeId && (
            <div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Card className="p-6">
                      ...
                      <p className="text-4xl font-bold text-foreground">{employee.performance_reviews?.[0]?.attendance_score ?? 'N/A'}%</p>
                    </Card>
                    <Card className="p-6">
                      <p className="text-sm text-muted-foreground mb-1">Reliability Score</p>
                      <p className="text-4xl font-bold text-foreground">{employee.performance_reviews?.[0]?.reliability_score ?? 'N/A'}%</p>
                    </Card>
                </div>
              ...
              <div className="space-y-6">
                ...
                <Button className="w-full justify-start gap-2" onClick={() => handleActionClick("Request Time Off")}><Calendar/>Request Time Off</Button>
                <Button className="w-full justify-start gap-2" variant="outline" onClick={() => handleActionClick("Update Availability")}><User/>Update Availability</Button>
              </div>
            </div>
          </>
        )}
